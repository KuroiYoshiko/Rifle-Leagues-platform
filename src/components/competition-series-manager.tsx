...
function CompetitionSeriesRow({ organisation, series, canManage }: {
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
      <div className="min-w-0">
        ...
        {canManage ? <>
          ...
          <h3 className="mt-3 text-base font-semibold text-foreground">{series.name}</h3>
          <p className="mt-1 text-sm text-muted-foreground">Owner access is required to rename or archive Competition Series.</p>
        </>}
      </div>
...
export function CompetitionSeriesManager({ organisation, series, canManage = true }: {
  organisation: { id: number; slug: string };
  series: CompetitionSeriesManagementRow[];
  canManage?: boolean;
}) {
  ...
  return <div className="space-y-3">{series.map((item) => <CompetitionSeriesRow key={item.id} organisation={organisation} series={item} canManage={canManage} />)}</div>;
}
