  ...
  return <div className="min-w-0 space-y-6">
    ...
    {data.groups.map(group => {
      ...
      <div role="region" aria-labelledby={`rr-division-${group.id}`} tabIndex={0} className="results-table-region relative max-w-full overflow-x-auto rounded-xl border border-border bg-surface outline-none focus-visible:ring-2 focus-visible:ring-brand">
        <table style={{ "--results-round-count": data.rounds.length } as CSSProperties} className="results-score-table results-round-robin-table w-full table-fixed border-separate border-spacing-0 text-[11px] tabular-nums">
          ...
          <thead><tr className="text-xs text-muted-foreground">
            <th scope="col" className="results-sticky-position sticky left-0 z-30 border-b border-r border-border bg-surface-muted px-1 py-2 text-center text-[10px]">Pos</th>
            <th scope="col" className="results-sticky-entrant sticky left-10 z-20 border-b border-r border-border bg-surface-muted px-2 py-2 text-left text-[10px]">Entrant</th>
            <th scope="col" className="results-average-cell hidden border-b border-border bg-surface-muted px-1 py-2 text-center text-[10px] lg:table-cell"><abbr title="Starting Average" className="no-underline">S/Av</abbr></th>
            <th scope="col" className="results-average-cell hidden border-b border-border bg-surface-muted px-1 py-2 text-center text-[10px] lg:table-cell"><abbr title="Running Average" className="no-underline">R/Av</abbr></th>
            {data.rounds.map(round => <th key={round.id} scope="col" className="border-b border-l border-border bg-surface-muted px-1 py-1.5 text-center">
              ...
              <time data-round-date-heading dateTime={round.deadline} aria-label={`Round End ${round.deadline}`} className="mt-0.5 block whitespace-nowrap text-[8px] font-normal">{date.format(new Date(`${round.deadline}T00:00:00Z`))}</time>
              {!round.released ? <span className="block text-[8px] font-normal">Unreleased</span> : null}
            </th>)}
            <th scope="col" className="border-b border-l border-border bg-brand-subtle px-1 py-2 text-center text-[10px] text-brand-deep">Match pts</th>
          </tr></thead>
          <tbody>{group.entrants.map(entrant => <Fragment key={entrant.entrant_id}>
            <tr data-entrant-row={entrant.entrant_id} className="[&:has(details[open])+tr]:table-row">
              <td className="results-sticky-position sticky left-0 z-20 border-b border-r border-border bg-surface px-1 py-2 text-center align-top text-[10px] font-semibold text-muted-foreground">{data.released_round_count ? `${entrant.position}${entrant.tied ? "=" : ""}` : "—"}</td>
              <th scope="row" className="results-sticky-entrant sticky left-10 z-10 border-b border-r border-border bg-surface px-2 py-2 text-left align-top font-normal">
                <div className="min-w-0 break-words"><span className="block text-[12px] font-semibold leading-tight">{name(entrant)}</span><span className="mt-1 block text-[9px] leading-tight text-muted-foreground">{entrant.club_name}</span><span className="results-inline-averages lg:hidden">{entrant.entrant_format === "individual" ? individualAverageSummary(entrant) : null}</span>
                  {entrant.entrant_format !== "individual" ? <details className="mt-1">
                    <summary aria-controls={`rr-participants-${group.id}-${entrant.entrant_id}`} className="min-h-8 cursor-pointer content-center rounded py-1 text-[10px] font-semibold text-brand-strong outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand">
                      Participants <span className="sr-only">for {entrant.entrant_label}, {entrant.club_name}</span>
                    </summary>
                  </details> : null}
                </div>
              </th>
              <td className="results-average-cell hidden border-b border-border px-1 py-2 text-center text-[11px] lg:table-cell">{entrant.entrant_format === "individual" && entrant.participants[0] && Object.hasOwn(entrant.participants[0], "starting_average") && entrant.participants[0].starting_average != null ? formatAverage.format(entrant.participants[0].starting_average) : "—"}</td>
              <td className="results-average-cell hidden border-b border-border px-1 py-2 text-center text-[11px] lg:table-cell">{entrant.entrant_format === "individual" && entrant.participants[0]?.running_average != null ? formatAverage.format(entrant.participants[0].running_average) : "—"}</td>
              {data.rounds.map(round => {
                ...
                return <td key={round.id} data-round-id={round.id} className="border-b border-l border-border px-1 py-2 text-center align-top leading-tight">
                  <div title={opponent ? `Versus ${opponent.full}` : undefined} className="mx-auto truncate text-[8px] text-muted-foreground">{opponent ? <><span aria-hidden="true">v {opponent.compact}</span><span className="sr-only">Versus {opponent.full}</span></> : cell ? "Bye" : "No fixture"}</div>
                  {!cell || cell.outcome === "pending" ? <span className="mt-1 block text-[10px] text-muted-foreground">Pending</span> : <>
                    <div className="mt-1 text-[12px] font-semibold">{cell.state === "nsr" ? <abbr title="No score returned: incomplete at Round End" className="no-underline">NSR</abbr> : number(cell.gun_score)}{data.uses_x_score && cell.state === "scored" ? <span className="ml-0.5 text-[9px] font-normal text-muted-foreground">{number(cell.x_total)}X</span> : null}</div>
                    <span className="mt-0.5 block whitespace-nowrap text-[9px]">{outcomes[cell.outcome]}{cell.match_points == null ? " · —" : ` · ${cell.match_points}p`}</span>
                  </>}
                </td>;
              })}
              <td className="border-b border-l border-border bg-brand-subtle/30 px-1 py-2 text-center align-top leading-tight"><span className="block text-[12px] font-bold text-brand-deep">{entrant.total_match_points} pts</span><span className="mt-0.5 block text-[9px] text-muted-foreground">{number(entrant.gun_total)} result{data.uses_x_score ? ` · ${number(entrant.x_total)}X` : ""}</span></td>
            </tr>
