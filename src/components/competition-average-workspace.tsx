...
function CompetitionSettings({ organisation, season, competition, configuration, management }: {
  ...
  const readOnly = management.hasFrozenStartingAverages;
  ...
  return <section aria-labelledby="competition-average-settings-heading">
    ...
    <Card className="p-5 sm:p-6">
      ...
      {readOnly ? <p className="mt-4 rounded-xl border border-warning/20 bg-warning-subtle p-4 text-sm text-warning">These settings are read-only because a Starting Average is frozen. Stage 2A does not change or unfreeze it.</p> : <form action={action} className="mt-5 space-y-4 border-t border-border pt-5">
...
function StartingAverageRow({
  ...
  return <div className="p-5">
    ...
    {(needsManual || (displayedOrigin === "manual" && displayedStatus === "provisional")) ? <form action={action} className="mt-3 grid gap-3 rounded-xl bg-surface-muted p-3 sm:grid-cols-[minmax(8rem,0.35fr)_minmax(0,1fr)_auto] sm:items-end">
      <CompetitionFields organisation={organisation} season={season} competition={competition} />
      <input type="hidden" name="competition_entrant_participant_id" value={participant.competitionEntrantParticipantId} />
      <label className="text-xs font-medium text-foreground">Starting Average<input name="starting_average" required type="text" inputMode="decimal" defaultValue={displayedAverage ?? ""} disabled={pending || displayedStatus === "frozen"} placeholder="97.50" className={inputClassName} /><span className="mt-1 block text-[11px] text-muted-foreground">Scale: {formatAverageMaximum(basisMaximum)}</span></label>
      <label className="text-xs font-medium text-foreground">Reason (optional)<input name="manual_reason" maxLength={500} defaultValue={state.manualReason ?? participant.manualReason ?? ""} disabled={pending || displayedStatus === "frozen"} className={inputClassName} /></label>
      <button type="submit" disabled={pending || displayedStatus === "frozen"} className="min-h-11 rounded-xl border border-border bg-surface px-4 text-sm font-semibold text-brand-deep transition hover:bg-brand-subtle disabled:opacity-60">{pending ? "Saving…" : "Save manual S/Av"}</button>
