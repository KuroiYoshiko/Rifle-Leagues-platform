...
function StartingAverageRow({
  ...
  return <div className="p-5">
    <div className="grid gap-3 text-sm md:grid-cols-[minmax(0,1.1fr)_0.5fr_minmax(0,1.5fr)_0.45fr_0.55fr] md:gap-4">
      ...
      <div><Badge tone={displayedStatus === "frozen" ? "positive" : needsManual && !manualSaved ? "warning" : displayedAverage === null ? "neutral" : "brand"}>{displayedStatus === "frozen" ? "Finalised" : needsManual && !manualSaved ? "Needs input" : notCalculated ? "Not calculated" : noHistory ? "No average" : displayedOrigin === "manual" ? "Manual · Provisional" : "Calculated · Provisional"}</Badge></div>
    </div>
