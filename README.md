...
### Standalone development demo bootstrap

The repository includes a complete fictional development world for a fresh
Supabase project. It is for development/test projects only and must not be run
against production.

Run the application schema files in this exact order:
...
Next, make a service-role or secret key available only to the one-off local
process. Never commit it or expose it through a `NEXT_PUBLIC_` variable:

```dotenv
SUPABASE_SERVICE_ROLE_KEY=your-development-project-service-role-key
RIFLELEAGUES_DEMO_PASSWORD=RifleLeagues-Demo-2026!
```

The helper also accepts `SUPABASE_URL`; when it is absent it uses the existing
`NEXT_PUBLIC_SUPABASE_URL`. Create or refresh the 40 login-capable, confirmed
demo accounts with:
...
Finally, run all of `database/development-demo-seed.sql` in the Supabase SQL
Editor. The Auth helper must run after `database/user-profiles.sql` so the
existing new-user trigger creates the matching profile for every account.

Unless `RIFLELEAGUES_DEMO_PASSWORD` overrides it, every demo account uses the
development-only shared password `RifleLeagues-Demo-2026!`. Useful logins are:

| Email | Primary testing context |
| --- | --- |
| `basildon.demo01@example.com` | Eastern organisation owner and Basildon club owner |
| `basildon.demo02@example.com` | Eastern organisation manager and Basildon official |
| `northbridge.demo01@example.com` | Northbridge club owner |
| `basildon.demo03@example.com` | Basildon club official |
| `basildon.demo04@example.com` | Normal Basildon member, entered shooter, and multi-club member |
| `westmere.demo01@example.com` | Central organisation owner and Westmere club owner |
| `southessex.demo02@example.com` | Central organisation manager and South Essex official |

Both bootstrap steps are rerunnable. The Auth helper creates missing marked
demo users and refreshes only existing users carrying the RifleLeagues demo
marker; it aborts on an email collision with an unmarked account. The SQL uses
stable slugs and unique relationships, and reconstructs only entrant and
division children belonging to the named demo competitions. It never truncates
tables, disables constraints, changes RLS, or weakens production security.

No demo reset helper is included. A reset of a disposable project should use a
fresh project or the normal Supabase local reset workflow rather than adding a
destructive script to the application repository.
