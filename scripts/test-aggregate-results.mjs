...
test("actual Competition route -> Supabase SSR RPC -> SQL -> rendered cells: Summer Pairs 200", async () => {
  ...
  for (const [index, row] of renderedRows.entries()) {
    ...
    assert.match(row, new RegExp(`data-entrant-position="true"[\\s\\S]*?>${index + 1}<`));
    assert.match(row, new RegExp(`>${index + 3}<span class="sr-only"> gun result`));
    assert.match(row, new RegExp(`data-total-cell="true"[\\s\\S]*?>${index + 3}<span class="sr-only"> total shooting result`));
    assert.match(row, new RegExp(`>${2 - index} pts<span class="sr-only"> total aggregate ranking points`));
...
test("points-scored final Total leads with the gun result and retains ranking points", async () => {
  ...
  const gunTotal = first.indexOf('>397<span class="sr-only"> total shooting result');
  const rankingTotal = first.indexOf('>5 pts<span class="sr-only"> total aggregate ranking points');
  assert.ok(gunTotal >= 0 && rankingTotal > gunTotal, "Aggregate final must lead with the shooting result");
  assert.match(html, />Total<\/th>/);
  assert.doesNotMatch(html, />Pos<\/th>/);
...
test("live source totals reproduce 397/396 through the UI; corrected achieved totals yield 3/4", async () => {
  ...
  assert.ok(rows[0].includes("Pair 2") && rows[0].includes('>396<span class="sr-only"> total shooting result'));
  assert.ok(rows[1].includes("Pair 1") && rows[1].includes('>397<span class="sr-only"> total shooting result'));
  ...
  assert.ok(correctedRows[0].includes("Pair 1") && correctedRows[0].includes('>3<span class="sr-only"> total shooting result'));
  assert.ok(correctedRows[1].includes("Pair 2") && correctedRows[1].includes('>4<span class="sr-only"> total shooting result'));
...
test("equal gun results share competition rank and points: 5,5,3,2,1", async () => {
  ...
  const competition = (await db.query("select * from competitions where id=1")).rows[0];
  const { html } = await renderAggregateResultsRoute({ competition, readRpc: () => data });
  assert.equal([...html.matchAll(/data-entrant-position="true"[^>]*>1=<\/span>/g)].length, 2);
