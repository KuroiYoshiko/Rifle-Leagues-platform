-- Canonical fresh-install schema: club operations.
-- Contains final current definitions only; historical upgrades live under database/upgrades/.

begin;
set local check_function_bodies = false;

CREATE OR REPLACE FUNCTION public.get_club_operational_summaries(p_club_id bigint DEFAULT NULL::bigint, p_warning_days integer DEFAULT 7)
 RETURNS TABLE(club_id bigint, club_name text, club_slug text, active_member_count bigint, active_competition_count bigint, attention_status text, outstanding_score_count bigint, competition_id bigint, competition_name text, competition_slug text, organisation_slug text, league_season_slug text, competition_round_id bigint, round_number integer, local_cutoff date, days_until_cutoff integer, participant_count bigint, complete_participant_count bigint, incomplete_participant_count bigint)
 LANGUAGE plpgsql
