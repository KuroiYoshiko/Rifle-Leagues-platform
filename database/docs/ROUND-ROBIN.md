...
`database/11_round_robin.sql` contains the RLS-protected table,
...
The same canonical file contains the narrow
...
- Consolidated schema definitions into `database/11_round_robin.sql`,
  `database/dev/development-round-robin-fixture.sql`, and this report.
...
Fresh installs receive the final table, generator, guards, and public Results RPC
from **`database/11_round_robin.sql`** as part of the complete numbered schema.
The file is not a standalone upgrade. Development databases may then run
**`database/dev/development-round-robin-fixture.sql`**.
