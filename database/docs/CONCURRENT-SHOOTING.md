...
`database/15_concurrent_shooting.sql` adds only four narrow functions:
...
Fresh installs receive structured shooting details from
`database/14_shooting_details.sql` and final Concurrent Shooting definitions
from `database/15_concurrent_shooting.sql` as part of the complete numbered
schema. Neither file is a standalone upgrade. A database already at the complete
pre-cleanup schema needs no SQL transition for this repository cleanup.

The canonical definitions create no groups, do no historical backfill or source
merge, and infer no relationship. Stage 2 behavior must be
present before Active groups are used for score entry; Stage 3A behavior must be
