...
Fresh installs receive the final Series definitions from
`database/12_series.sql`, followed by structured shooting definitions in
`database/14_shooting_details.sql` and global integrity/security in
`database/18_security_and_integrity.sql`. Run the complete numbered schema in
numeric order; these files are not standalone upgrade scripts.

An existing database that already had the complete pre-cleanup chain applied
needs no SQL transition for the directory cleanup. Follow `database/upgrades/`
for future versioned transitions instead of replaying historical Stage files.
...
The canonical `database/14_shooting_details.sql` subsystem extends that contract
...
controls. The canonical `database/12_series.sql` contains the narrow
...
`database/dev/development-competition-series-fixture.sql`. It adds only marker-named,
