-- Canonical fresh-install schema: shooter analytics.
-- Contains final current definitions only; historical upgrades live under database/upgrades/.

begin;
set local check_function_bodies = false;

CREATE OR REPLACE FUNCTION public.get_my_shooter_analytics(p_season_id bigint DEFAULT NULL::bigint, p_equipment_kind text DEFAULT NULL::text, p_equipment_code text DEFAULT NULL::text, p_equipment_custom_id bigint DEFAULT NULL::bigint, p_position_mode text DEFAULT NULL::text, p_position_code text DEFAULT NULL::text, p_position_custom_id bigint DEFAULT NULL::bigint, p_distance_mode text DEFAULT NULL::text, p_distance_value numeric DEFAULT NULL::numeric, p_distance_unit text DEFAULT NULL::text, p_history_page integer DEFAULT 1, p_include_if_seeded_today boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
